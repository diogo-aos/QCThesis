...
class K_Means:       
    ...
    def fit_debug(self, data, K, iters=3, mode="cuda", cuda_mem='manual',tol=1e-4,max_iters=300):
        ...
        while not stopcond:
            ...
            self.centroids =  self._np_recompute_centroids(data,self.centroids,self.labels2)
    ...
    def fit(self, data, K, iters=3, mode="cuda", cuda_mem='manual',tol=1e-4,max_iters=300):
        ...
        while not stopcond:
            ...
            #assign,grouped_data = self._assign_data(data,dist_mat)
            self.centroids =  self._np_recompute_centroids(data,self.centroids,labels)
    ...
    def _label(self,data,centroids):
        if self._mode == "cuda":
            dist_mat = self._cu_label(data,centroids,gridDim=None,
                                           blockDim=None)#,memManage='manual')
        elif self._mode == "numpy":
            dist_mat = self._np_label_fast(data,centroids)

        elif self._mode == "python":
            dist_mat = self._np_label(data,centroids)
    ...
    def _np_recompute_centroids_groups(self,data,centroids,labels):
        ...
        print "Array creation & mean",end            
            
        return new_centroids

    def _np_recompute_centroids_new(self,data,centroids,labels):
        ...
        for k in xrange(K):
            new_centroids[k] = sortedData[k].sum(axis=0) #this is wrong
    ...
    def _np_recompute_centroids(self,data,centroids,labels):
        """
        INEFFICIENT
        """
        # change to get dimension from class or search a non-empty cluster
