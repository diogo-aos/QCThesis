...
The first step is inherently parallel as the computation of the label of the $n-th$ pattern is not dependent on any other pattern, but only on the centroids. Two possible approaches to parallelize this step on the \gls{gpu} are possible, a data-centric and a centroid-centric. In the former, a parallel computation unit will compute the distance from one pattern to all centroids and assign the pattern to the closest centroid. In the later, each computation unit will 

\subsubsection{Parallel Hierarchical Agglomerative Clustering}

Hierarchical Agglomerative Clustering is an algorithm that is not easily parallelized since it has several dependencies 
