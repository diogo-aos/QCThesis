...
Experiments showed that building the matrix with the recommended sparse structures for building the matrix saturated the memory and were very slow. Using a CSR matrix didn't saturate memory but was too slow. % TODO add reference to experiments in results
...
% #TODO add all the text in the notebook
...
\begin{algorithmic}[1]
\Procedure{update\_cluster}{$indices, data, indptr, degree, cluster, max\_assocs$}
\State{$n\_ptr\_id = new\_assocs\_idx[n\_ptr]$}
\State{$i\_ptr\_id = indices[i\_ptr]$}
\While{$o\_ptr \ge fa$}
	\If{$i\_ptr\_id > n\_ptr\_id$}
		\State{$indices[o\_ptr] = indices[i\_ptr]$}
		\State{$data[o\_ptr] = data[i\_ptr]$}
		\State{$o\_ptr -= 1$}
		\State{$i\_ptr -= 1$}
		\State{$continue$}
	\EndIf
	\State{$idx = new\_assocs\_idx[n\_ptr]$}
	\If{$i\_ptr \ge idx$}
		\State{$indices[o\_ptr] = indices[i\_ptr]$}
		\State{$data[o\_ptr] = data[i\_ptr]$}
		\State{$o\_ptr -= 1$}
